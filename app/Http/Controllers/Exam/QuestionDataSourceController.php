    ...
    public function fetchDataBySubject(Request $request, $exam, $subject)
    {
        if($request->ajax())
        {
            try {
                ...
                $questions = Question::with('subject','user','questionType')->whereNotIn('id',$exam_question_ids)->where('subject_id',$subject)->select('id','question_type_id','title','excerpt')->paginate(25);
